...
public class ToDoItemClientAdminService(HttpClient httpClient) : IToDoItemAdminService
{
    ...
    {
        var result = await _httpClient.PostAsJsonAsync("/api/ToDoItem", LowercaseNamePlaceholder);
    ...
    {
        var result = await _httpClient.PutAsJsonAsync($"/api/ToDoItem/{id}", LowercaseNamePlaceholder);
